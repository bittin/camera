...
def read_translations() -> dict[str, dict[str, str]]:
    ...
    some of them directly (`app-name` becomes the launcher name, `camera` its
    generic name) and references others inside its own strings, so they all
...
def render_locale_readme(lang: str, images: list[str], keys: list[str],
    ...
    return "\n".join([
        ...
        "",
        f"# {translations['app-name'][SOURCE_LANG]} ({lang})",
        "",
...
def render_preview_readme(translations: dict[str, dict[str, str]], metainfo: str) -> str:
    ...
    return "\n".join([
        ...
        "",
        f"# {translations['app-name'][SOURCE_LANG]} screenshots",
        "",
