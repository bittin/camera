...
#
# The app is named for what it is, so Name is translated and the launcher lists
# it the way GNOME's Snapshot lists itself: as "Camera" in the user's own
# language. GenericName would have carried the same word, but no current
# launcher displays it.
DESKTOP_KEYS = {
    "Name": "camera",
...
#
# <name> takes the same string the desktop entry's Name does, so the stores and
# the launcher agree on what the app is called.
METAINFO_HEAD = {
    "name": ["camera"],
