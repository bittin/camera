#!/usr/bin/env python3
"""Inject translations from i18n/*/klikka.ftl into the derived metadata files.
...
# One Fluent file per language, named after the crate: i18n-embed-fl derives the
# file name from CARGO_PKG_NAME, so renaming the package renames these too.
FTL = "klikka.ftl"
...
def read_translations() -> dict[str, dict[str, str]]:
    ...
    rather than a translated product name reaching the launcher.
    """
    ...
    if APP_NAME_KEY in out:
        langs = ", ".join(sorted(out[APP_NAME_KEY]))
        sys.exit(
            f"error: `{APP_NAME_KEY}` is defined in i18n/{{{langs}}}/{FTL}. The "
            f"application name is not translatable; remove it and edit APP_NAME "
            f"in this script and src/constants.rs instead."
        )
    out[APP_NAME_KEY] = {SOURCE_LANG: APP_NAME}
    return out
...
def resolve_references(translations: dict[str, dict[str, str]]) -> None:
    ...
    def expand(key: str, lang: str, seen: frozenset[str]) -> str:
        if key in seen:
            sys.exit(f"error: circular reference to {{ {key} }} in i18n/{lang}/{FTL}")
        values = translations.get(key)
        if not values:
            sys.exit(f"error: unknown reference {{ {key} }} in i18n/{lang}/{FTL}")
...
def render_locale_readme(lang: str, images: list[str], keys: list[str],
    ...
    return "\n".join([
        ...
        f"i18n/{lang}/{FTL} and run `just generate`. -->",
        "",
        f"# {APP_NAME} ({lang})",
        "",
...
def render_preview_readme(translations: dict[str, dict[str, str]], metainfo: str) -> str:
    ...
    return "\n".join([
        f"<!-- Generated by scripts/{Path(__file__).name}. Edit the captions in "
        f"i18n/{SOURCE_LANG}/{FTL} and run `just generate`. -->",
        "",
        f"# {APP_NAME} screenshots",
        "",
...
def main() -> int:
    ...
    if missing := expected - source_keys:
        sys.exit(f"error: missing from i18n/{SOURCE_LANG}/{FTL}: {', '.join(sorted(missing))}")
    if extra := source_keys - expected:
        sys.exit(f"error: unmapped keys in i18n/{SOURCE_LANG}/{FTL}: {', '.join(sorted(extra))}")
