...
// GPU compute shader for YUYV to RGBA conversion
//
// YUYV: Packed 4:2:2 - each 4 bytes encode 2 pixels: [Y0 U Y1 V]
// Texture is uploaded as RGBA8 where: R=Y0, G=U, B=Y1, A=V
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let is_odd = (x & 1u) == 1u;
    let luma = select(packed.r, packed.b, is_odd);

    // U and V are shared between pixel pairs
    let rgb = yuv_to_rgb_bt601(luma, packed.g, packed.a);
